...
if (-not $winapp) {
    ...
    if (Test-Path $packageRoot) {
        $winapp = Get-ChildItem -Path $packageRoot -Recurse -Filter "winapp.exe" |
            Where-Object { $_.FullName -like "*\tools\win-$rid\winapp.exe" } |
...
$winappPath = if ($winapp.Source) { $winapp.Source } else { $winapp.FullName }
