...
$hasNuGetAudit = $extraArgs | Where-Object { $_ -match "^[/|-]p:NuGetAudit=" }
...
if ($DisableNuGetAudit -and -not $hasNuGetAudit) { $autoArgs += "/p:NuGetAudit=false" }
...
. (Join-Path $PSScriptRoot "scripts\FindWinApp.ps1")
$winappPath = Find-WinApp -RuntimeIdentifier "win-$rid"

if (-not $winappPath) {
    Write-Host "WARNING: winapp CLI not found in PATH or NuGet package cache -- skipping run" -ForegroundColor Yellow
...
if ($Detach) {
    ...
    Write-Host "--> Launching app: $winappPath run $outputDir --debug-output" -ForegroundColor Cyan
