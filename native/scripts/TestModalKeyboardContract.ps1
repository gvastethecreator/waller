...
foreach ($check in @(
    ...
    @{ Text = $mainPage; Pattern = 'ViewModel\.Presets\.PropertyChanged\s*\+=\s*OnPresetsPropertyChanged'; Message = "MainPage must use the focused Presets handler for Preset modal routing." },
    ...
    @{ Text = $mainPage; Pattern = 'nameof\s*\(\s*PresetsViewModel\.ManagePresetsVisibility\s*\).*ViewModel\.Presets\.IsManagePresetsOpen[\s\S]*FocusWhenReady\s*\(\s*ManagePresetsModal\.FocusPresetList\s*\)'; Message = "Manage Presets opening must focus the preset list." },
    @{ Text = $mainPage; Pattern = 'nameof\s*\(\s*PresetsViewModel\.SaveAsVisibility\s*\).*ViewModel\.Presets\.IsSaveAsOpen[\s\S]*FocusWhenReady\s*\(\s*SaveAsModal\.FocusPresetName\s*\)'; Message = "Save As opening must focus the preset name input." },
    ...
    @{ Text = $mainPage; Pattern = 'nameof\s*\(\s*PresetsViewModel\.DeleteConfirmationVisibility\s*\).*ViewModel\.Presets\.IsDeleteConfirmationOpen[\s\S]*FocusWhenReady\s*\(\s*ManagePresetsModal\.FocusConfirmDelete\s*\)'; Message = "Delete confirmation opening must focus the confirm delete action." },
